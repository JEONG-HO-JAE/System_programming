...
void change_rodata(int32_t fd, Elf64_Shdr sh_table) {
	...
	//Create buffer for rodata
	char* buff = read_section(fd, sh_table);

    const char *target = "software";
    const char *replacement = "hackers!";
	size_t target_len = strlen(target);
	size_t replacement_len = strlen(replacement);

	//Check whether the "software" is in rodata
	//If it is true, change to hackers!
	for (size_t i = 0; i < sh_table.sh_size; ++i) {
        if (memcmp(buff + i, target, target_len) == 0) {
            memcpy(buff + i, replacement, replacement_len);
            printf("Replaced 'software' with 'hackers!'\n");
        }
    }

	lseek(fd, sh_table.sh_offset, SEEK_SET);
    write(fd, buff, sh_table.sh_size); 
